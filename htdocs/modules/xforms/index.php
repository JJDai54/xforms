...
// JJDai - si le formulaire nécéssite une réponse creation d'un tableau pour creer un enregistrement dans la table userform
...
//    echo "eleArray (contient les donnees saisies): <pre>" . print_r($ele , true) . "</pre><hr>";
        ...
        if (!empty($ele[$eleId])) {
            if ('' !== $eleCaption) {
                $tplTitle = "- <b>%s</b> : "; //JJDai - gras a ajouter en paramettre des formulaire peut être
            ...
            switch ($eleType) {
                ...
                case 'text':
                   ...
                   */
                    // JJDai - ajout des info dans le cas d'un formulaire nécéssitant une réponse comme un formulaire de contact
...
//                        }
    ...
    //-----------------------------------------
    //Enregistrement des infos si le formulaire nécéssite une réponse
    if (!is_null($uform)) {
      if (count($err) == 0) {
        $uform->unsetNew();
        $uFormHandler->insert($uform);
...
//        $msg = strip_tags(htmlspecialchars_decode($msg), '<href>');
    ...
    if ($sendCopy && (0 == count($err))) {
        $emailstoCopy = array();
        //modif JJDai : l'email saisi n'est pas forcement celui de l'utilisateur connecté
        //aussi il faut d(abord vérifier si un email a ete saisi)
...
//echo "<hr>Liste des formulaires : <pre>". print_r($_POST, true)  . "</pre>";
    ...
    //--------------------------------------------
    // retour avec persistance des données saisies
